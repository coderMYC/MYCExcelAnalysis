...
#import "MYCExcelWookbook.h"
#import "ZipArchive.h"
#import "XMLReader.h"
@interface MYCExcelWookbook()
...
@implementation MYCExcelWookbook
                ...
                if(writeSuccess)
                {
                    ...
                    if(uncompressionSuccess)
                    {
                        ...
                        for(NSInteger i = 0 ; i < array.count ; i ++)
                        {
                            ...
                            MYCExcelSheet *sheet = [[MYCExcelSheet alloc]init];
                            
                            sheet.sheetId = [[sheetDic objectForKey:@"sheetId"] integerValue];
                            
                            sheet.cellArray = [MYCExcelSheet analysisSheetDataWithSheetDic:sheetDic sharedStringsArray:stringValueArray];
    ...
    while (fileName = [dirEnum nextObject]) {
        ...
        if ([fileName hasSuffix:@".xml"]) {
            ...
            if(xmlData && xmlData.length > 0)
            {
                ...
                NSInteger sheetId = [[MYCExcelCell getNumberFromStr:fileName] integerValue];
